...
<div id="upload_price_container" class="avo_style_upload_price_container">
    <div class="avo_style_uploadTopBox">
        <form id="up_price_upload_form" target="upload_target" enctype="multipart/form-data" method="post" action="{$SITE_PATH}/dyn/servicecompany/do_upload_service_company_price" style="float:left">
            <div style="padding: 10px;">
                {if ($ns.userLevel === $ns.userGroupsAdmin)}
                    <div class="avo_style_titleBox">
                        <label for="up_selected_service_company">{$ns.langManager->getPhraseSpan(66)}:</label>
                    </div>
                    <select onkeyup="this.blur();
							this.focus();" class="cmf-skinned-select cmf-skinned-text avo_style_up_selected_company" name='up_selected_service_company' id='up_selected_service_company' autocomplete="off">
                        {html_options values=$ns.companiesIds  selected=$ns.selectedCompanyId  output=$ns.companiesNames}
                    </select>
                {else}
                    <input  id="up_selected_service_company" name="up_selected_service_company" type="hidden" value="{$ns.userId}"/>
                {/if}
                <div class="avo_style_titleBox">
        ...
        {/if}
		<a  href="{$SITE_PATH}/price/service_last_price/{$company_id}"> 
            ...
            <div class="avo_style_uploadWrapperBox">
                <table cellspacing="0" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <td ><a href="{$SITE_PATH}/price/service_zipped_price_unzipped/{$company_price->getId()}"> <img src = "{$SITE_PATH}/img/zip_file_download.png"  alt="zip"/> </a></td>
